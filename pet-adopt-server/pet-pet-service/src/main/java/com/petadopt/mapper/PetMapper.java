...
public interface PetMapper {
    String BASE = " from pet p left join pet_category c on p.category_id=c.id left join pet_category s on p.subcategory_id=s.id where p.deleted=0 ";

    @Select({"<script>",
        "select p.*,c.name category_name,s.name subcategory_name ", BASE,
    ...
    @Select("select p.*,c.name category_name,s.name subcategory_name " +
            "from pet p left join pet_category c on p.category_id=c.id left join pet_category s on p.subcategory_id=s.id " +
            "where p.id=#{id} and p.deleted=0")
    Pet findById(Long id);

    @Select({"<script>",
        "select id,owner_id,name,cover_image,status from pet where deleted=0 and id in",
